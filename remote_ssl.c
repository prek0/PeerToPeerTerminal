...
int main(int argc, char *argv[]) {
    ...
    // Create 2 pipes for inter-process communication
    pipe(pipeIn);
    pipe(pipeOut);
    ...
    if (pid1 == 0){
        ...
        while(1){
            ...
            if(pfd[0].revents & POLLIN){
                ...
                bufferIn[valread] = '\0';
                // Write data (command to be executed) to input pipe for child process
                if (write(pipeIn[1], bufferIn, strlen(bufferIn)) == 0){
                    break;
                }
            }               
            // Check for data to read from child process output(pipeOut)
            if(pfd[1].revents & POLLIN){
                size = read(pipeOut[0], bufferOut, sizeof(bufferOut) - 1);
                if (size > 0){
                    bufferOut[size] = '\0';
                    // Write data (output of the terminal commands) to SSL connection
